...
from alert_rules import alert_engine, AlertRule, RuleType, NotificationType, AlertStatus
...
class RuleConditionModel(BaseModel):
    rule_type: str = "keyword_frequency"
...
class AlertRuleCreateModel(BaseModel):
    name: str
    description: str = ""
    container_id: Optional[str] = None
    container_name: Optional[str] = None
    condition: RuleConditionModel
    notifications: List[NotificationChannelModel] = []
    enabled: bool = True
    cooldown_seconds: int = 300


class AlertRuleUpdateModel(BaseModel):
    name: Optional[str] = None
    description: Optional[str] = None
    container_id: Optional[str] = None
    container_name: Optional[str] = None
    condition: Optional[RuleConditionModel] = None
    notifications: Optional[List[NotificationChannelModel]] = None
    enabled: Optional[bool] = None
    cooldown_seconds: Optional[int] = None


class MuteRuleModel(BaseModel):
...
async def startup_event():
    ...
    app_logger.info("Alert rule engine initialized")
...
async def list_alert_rules(
    container_id: Optional[str] = Query(None, description="按容器 ID 筛选规则")
):
    """获取告警规则列表"""
    try:
        rules = alert_engine.list_rules(container_id=container_id)
        return {
            "success": True,
            "data": [rule.to_dict() for rule in rules]
        }
    except Exception as e:
        log_error("list_alert_rules", e)
        raise


@app.post("/api/alert-rules")
async def create_alert_rule(rule_data: AlertRuleCreateModel):
    """创建告警规则"""
    try:
        rule_dict = rule_data.model_dump()
        rule = alert_engine.create_rule(rule_dict)
        return {
            "success": True,
            "data": rule.to_dict(),
            "message": "告警规则创建成功"
...
async def get_alert_rule(rule_id: str):
    ...
    try:
        ...
        log_error("get_alert_rule", e, rule_id=rule_id)
        raise


@app.put("/api/alert-rules/{rule_id}")
async def update_alert_rule(rule_id: str, rule_data: AlertRuleUpdateModel):
    """更新告警规则"""
    try:
        existing = alert_engine.get_rule(rule_id)
        if not existing:
            raise HTTPException(status_code=404, detail="告警规则不存在")
        ...
        rule = alert_engine.update_rule(rule_id, existing_dict)
        return {
            "success": True,
            "data": rule.to_dict(),
            "message": "告警规则更新成功"
        ...
        log_error("update_alert_rule", e, rule_id=rule_id)
        raise
...
async def delete_alert_rule(rule_id: str):
    ...
    try:
        ...
        log_error("delete_alert_rule", e, rule_id=rule_id)
        raise


@app.post("/api/alert-rules/{rule_id}/mute")
async def mute_alert_rule(rule_id: str, mute_data: MuteRuleModel):
    """静音告警规则"""
    try:
        success = alert_engine.mute_rule(rule_id, mute_data.duration_seconds)
        if not success:
            raise HTTPException(status_code=404, detail="告警规则不存在")
        return {
            "success": True,
            "message": f"告警规则已静音 {mute_data.duration_seconds} 秒"
        }
    except HTTPException:
        raise
    except Exception as e:
        log_error("mute_alert_rule", e, rule_id=rule_id)
        raise


@app.post("/api/alert-rules/{rule_id}/unmute")
async def unmute_alert_rule(rule_id: str):
    """取消静音告警规则"""
    try:
        success = alert_engine.unmute_rule(rule_id)
        if not success:
            raise HTTPException(status_code=404, detail="告警规则不存在")
        return {
            "success": True,
        ...
        log_error("unmute_alert_rule", e, rule_id=rule_id)
        raise
...
async def list_alerts(
    ...
    """获取告警事件列表"""
    try:
        alerts = alert_engine.list_alerts(status=status, rule_id=rule_id, limit=limit)
        return {
            "success": True,
            "data": [alert.to_dict() for alert in alerts]
        }
    except Exception as e:
        log_error("list_alerts", e)
        raise
...
async def get_alert(alert_id: str):
    ...
    try:
        ...
        log_error("get_alert", e, alert_id=alert_id)
        raise
